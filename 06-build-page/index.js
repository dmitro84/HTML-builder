...
(async function () {
    ...
    try {
        ...
        await copyFile(templateFile, path.join(__dirname, 'project-dist', 'index.html'));

        const filesCss = await readdir(path.join(__dirname, 'styles'), { withFileTypes: true });
        ...
        fs.readFile(path.join(__dirname, 'template.html'), (err, dataFile) => {
            ...
            for (const file of filesHtml) {
                ...
                if (file.isFile() && path.extname(file.name).slice(1) === 'html') {
                    ...
                    readStreamHtmlComponents = fs.createReadStream(path.join(path.join(__dirname, 'components'), file.name), 'utf-8');
...
async function listDirectory(dir) {
    ...
    for (const file of files) {
        if (file.isDirectory()) {
            ...
            await copyFile(path.join(dir, file.name), path.join(fileCopy, 'assets', prevNameDir, file.name));
        }
    }
